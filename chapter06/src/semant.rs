...
/// 语义分析的返回值，包括两部分：
///   - exp：语义分析和产生ir是耦合在一起的，exp是产生的ir
///   - ty：语义分析的结果，也就是表达式等的求值结果的类型
#[derive(Debug)]
pub struct ExpTy {
    pub exp: Exp,
    pub ty: Type,
...
impl<'a, F: Clone + Frame + PartialEq> SemanticAnalyzer<'a, F> {
        ...
        for typ in types {
            ...
            if let Ty::Name{ ref ident } = typ.node.ty.node {
                if names.contains(&ident.node) {
                    return self.add_error(Error::Cycle { pos: typ.node.ty.pos }, ());
                }
            }
        }
    }
    ...
    fn trans_dec(&mut self, declaration: &DeclarationWithPos, parent_level: &Level<F>) {
        match declaration.node {
            Declaration::Function(ref declarations) => {
                ...
                for (&WithPos { node: FuncDeclaration { ref params, ref body, ref result, .. }, .. }, ref level) in declarations.iter().zip(&levels) {
                    let result_type =
                        if let Some(ref result) = *result {
        ...
        match expr.node {
            Expr::Array { ref init, ref size, ref typ } => {
                ...
                match ty {
                    ...
                    Type::Error => (),
                    _ => return self.add_error(Error::UnexpectedType { kind: "array".to_string(), pos: typ.pos }, EXP_TYPE_ERROR),
                }
                ExpTy {
                    exp: (),
                    ty,
            ...
            Expr::Break => {
                if !self.in_loop {
                    return self.add_error(Error::BreakOutsideLoop { pos: expr.pos }, EXP_TYPE_ERROR);
                }
                ExpTy {
                    exp: (),
                    ty: Type::Unit,
            ...
            Expr::Int { .. } => ExpTy { exp: (), ty: Type::Int },
            ...
            Expr::Nil => ExpTy { exp: (), ty: Type::Nil },
            ...
            | Expr::Oper { ref left, oper: WithPos { node: oper @Operator::Divide, .. }, ref right } => self.check_binary_op(oper, left, right, level),
            ...
            Expr::Record { ref fields, ref typ } => {
                ...
                match ty {
                    Type::Record(_, ref type_fields, _) => {
                        ...
                        for field in fields {
                            let found = type_fields.iter().any(|&(type_field_name, _)| field.node.ident == type_field_name);
                            if !found {
                    ...
                    Type::Error => (),
                    _ => return self.add_error(Error::UnexpectedType { kind: "record".to_string(), pos: typ.pos }, EXP_TYPE_ERROR),
                }
                ExpTy {
                    exp: (),
                    ty,
            ...
            Expr::Str { .. } => ExpTy { exp: (), ty: Type::String },
    ...
    fn trans_var(&mut self, var: &VarWithPos, level: &Level<F>) -> ExpTy {
        match var.node {
                ...
                match var.ty {
                    Type::Record(record_type, ref fields, _) => {
                        for &(name, ref typ) in fields {
                            if name == ident.node {
                                return ExpTy { exp: (), ty: typ.clone() };
                            }
                        }
                        self.unexpected_field(ident, ident.pos, record_type)
                    },
                    typ => return self.add_error(Error::NotARecord { pos: this.pos, typ: typ }, EXP_TYPE_ERROR),
                }
            },
            Var::Simple { ref ident } => {
                if let Some(Entry::Var { ref typ, .. }) = self.env.look_var(ident.node).cloned() {
                    return ExpTy { exp: (), ty: self.actual_ty_var(typ) };
                }
            ...
            Var::Subscript { ref expr, ref this } => {
                ...
                match var.ty {
                    Type::Array(typ, _) => ExpTy { exp: (), ty: self.actual_ty_var(&typ) },
                    Type::Error => ExpTy { exp: (), ty: Type::Error },
                    typ => self.add_error(Error::CannotIndex { pos: this.pos, typ }, EXP_TYPE_ERROR),
                }
            },
        }
    }
    ...
    fn duplicate_param(&mut self, param: &FieldWithPos) {
        ...
        self.add_error(Error::DuplicateParam { ident, pos: param.pos }, ())
    }
    ...
    fn extra_field(&mut self, field: &RecordFieldWithPos, typ: &SymbolWithPos) -> ExpTy {
        ...
        self.add_error(Error::ExtraField { ident, pos: field.pos, struct_name }, EXP_TYPE_ERROR)
    }

    fn missing_field(&mut self, field_type: Symbol, typ: &SymbolWithPos) -> ExpTy {
        let ident = self.env.type_name(field_type);
        let struct_name = self.env.type_name(typ.node);
        self.add_error(Error::MissingField { ident, pos: typ.pos, struct_name }, EXP_TYPE_ERROR)
    }

    fn undefined_function(&mut self, ident: Symbol, pos: Pos) -> ExpTy {
        let ident = self.env.var_name(ident).to_string();
        self.add_error(Error::Undefined { ident, item: "function".to_string(), pos }, EXP_TYPE_ERROR)
    }

    fn undefined_identifier(&mut self, symbol: &SymbolWithPos) -> Entry<F> {
        let ident = self.env.type_name(symbol.node);
        self.add_error(Error::Undefined { ident, item: "identifier".to_string(), pos: symbol.pos }, Entry::Error)
    }

    fn undefined_type(&mut self, symbol: &SymbolWithPos) -> Type {
        let ident = self.env.type_name(symbol.node);
        self.add_error(Error::Undefined { ident, item: "type".to_string(), pos: symbol.pos }, Type::Error)
    }

    fn undefined_variable(&mut self, ident: Symbol, pos: Pos) -> ExpTy {
        let ident = self.env.var_name(ident).to_string();
        self.add_error(Error::Undefined { ident, item: "variable".to_string(), pos }, EXP_TYPE_ERROR)
    }
    ...
    fn unexpected_field(&mut self, ident: &SymbolWithPos, pos: Pos, typ: Symbol) -> ExpTy {
        ...
        self.add_error(Error::UnexpectedField { ident, pos, struct_name }, EXP_TYPE_ERROR)
    }
}
