...
impl<'a, F: Clone + Frame + PartialEq> SemanticAnalyzer<'a, F> {
    ...
    -> Result<Vec<Fragment<F>>> {
        ...
        let result = Some(WithPos::new(
            self.env.type_symbol("int"),
            pos,
        ));
        self.trans_dec(
            &WithPos::new(
    ...
    fn check_binary_op(
        ...
        let right = self.trans_exp(right, level, done_label.clone());
    ...
    pub fn trans_exp(
        ...
        match expr.node {
            ...
            Expr::If { ref else_, ref test, ref then } => {
                ...
                ExpTy {
                    exp: if_expression(
                        test_expr.exp,
                        if_expr.exp,
                        else_expr.map(|expr| expr.exp),
                        level,
                    ),
                    ty,
                }
            },
            Expr::Int { value } => ExpTy {
                exp : num(value),
                ty  : Type::Int,
            },
            Expr::Let {
            ...
            | Expr::Oper { ref left, oper: WithPos { node: oper @Operator::Divide, .. }, ref right } => self.check_binary_op(oper, left, right, level, done_label),
                ...
                if let Some((last_expr, exprs)) = exprs.split_last() {
                    ...
                    if new_exprs.is_empty() {
                        ...
                        for expr in new_exprs.into_iter().rev() {
                            exprs = Statement::Sequence(
                                Box::new(Statement::Exp(expr.exp)),
                                Box::new(exprs),
                            );
                        }
                        ExpTy {
    ...
    fn trans_var(
        ...
        match var.node {
            ...
            Var::Simple { ref ident } => {
                if let Some(Entry::Var { ref access, ref typ }) = self.env.look_var(ident.node).cloned() {
                    return ExpTy {
